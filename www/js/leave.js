...
angular.module('leave', ['starter', 'ngFileUpload']).controller('LeaveCtrl', function($scope, $http, $window, $ionicPlatform, $ionicModal, switchLan, Upload, $translate, $ionicPopup) {
  ...
  function sendEmail() {
    var res = $http({
      ...
      data: {
        ...
        subject: leaveDetails.type,
        text: leaveDetails.reason,
        html: leaveDetails.reason,
  ...
  $scope.submitLeave = function(l) {
    leaveDetails = l;
    var str;
    str = leaveDetails.startDate.toDateString();
    leaveDetails.type += ' (from ' + str + ' to ';
    str = leaveDetails.endDate.toDateString();
    leaveDetails.type += str + ')';
    leaveDetails.reason += '<p> Sent from ' + $scope.username;
